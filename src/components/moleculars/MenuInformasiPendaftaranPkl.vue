...
<template>
  <div>
    <div class="flex items-center space-x-3 font-semibold text-gray-900 text-xl leading-8 pt-2">
    ...
    <div class="grid grid-cols-2 xl:grid-cols-3" v-if="data">
      ...
      <router-link :to="{ name: 'ListProsesPengajuanTempatPKL' }">
        <ButtonDataTwo title="Proses Pengajuan Tempat Prakerin" :titleShort="data.prosesPengajuanTempatPKL" />
      </router-link>
      <router-link :to="{ name: 'ListProsesPenempatanPKL' }">
        <ButtonDataTwo title="Proses Penempatan Prakerin" :titleShort="data.prosesPenempatanPKL" />
      </router-link>
            ...
            <Popper content="Tombol Refresh Data" @click="getData()">
              <template #content>
                <div class="form-check form-switch">
                  <button class="bg-emerald-600 py-2 px-2 rounded-full text-gray-100 text-xs font-bold">
                    Refresh
                  </button>
                </div>
              </template>
