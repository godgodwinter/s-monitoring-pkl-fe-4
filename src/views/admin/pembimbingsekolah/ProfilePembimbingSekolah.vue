...
const getDataId = async () => {
  try {
    const response = await Api.get(`admin/pembimbingsekolah/${id}`);
...
<template>
  ...
  <div class="pt-4 px-10">
    <span class="text-2xl sm:text-3xl leading-none font-bold text-gray-700 shadow-sm">Pembimbing Sekolah</span>
  </div>
  ...
  <div class="container mx-auto my-5 p-5">
    <div class="md:flex no-wrap md:-mx-2">
      ...
      <div class="w-full md:w-9/12 mx-2 h-64">
        ...
        <div class="bg-white p-3 rounded-sm">
          ...
          <div class="text-gray-700">
            <div class="grid md:grid-cols-2 text-sm">
              ...
              <div class="grid grid-cols-2">
                <div class="px-4 py-2 font-semibold">Nomer Identitas / NIP</div>
        ...
        <div class="divider"></div>
        <div class="bg-white p-3" v-if="dataAsli.jurusan">
          <div class="flex items-center space-x-2 font-semibold text-gray-900 leading-8">
            <span clas="text-green-500">
              <svg class="h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <span class="tracking-wide">Kepala Jurusan</span>
          </div>
          <div class="text-gray-700">
            <div class="grid md:grid-cols-2 text-sm">
              <div class="grid grid-cols-2">
                <div class="px-4 py-2 font-semibold">Jurusan</div>
                <div class="px-4 py-2">{{ dataAsli.jurusan.nama }}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-if="dataAsli.tempatpkl">
          <div v-if="dataAsli.tempatpkl.length > 0">
            <div class="bg-white p-3" v-for="item, index in dataAsli.tempatpkl" :key="item.id">
              <div class="flex items-center space-x-2 font-semibold text-gray-900 leading-8">
                <span clas="text-green-500">
                ...
                <span class="tracking-wide">Tempat PKL {{ index + 1 }}</span>
              </div>
              <div class="text-gray-700">
                <div class="grid md:grid-cols-2 text-sm">
                  <div class="grid grid-cols-2">
                    <div class="px-4 py-2 font-semibold">Tempat PKL</div>
                    <div class="px-4 py-2">{{ item.tempatpkl.nama }}</div>
                  </div>
                  <!-- <div class="grid grid-cols-2">
                  ...
                  <div class="grid grid-cols-2">
                    ...
                    <div class="px-4 py-2">{{ moment(item.tempatpkl.tgl_mulai).format("DD MMMM YYYY") }}</div>
                  </div>
                  <div class="grid grid-cols-2">
                    <div class="px-4 py-2 font-semibold">Tanggal Selesai</div>
                    <div class="px-4 py-2">{{ moment(item.tempatpkl.tgl_selesai).format("DD MMMM YYYY") }}</div>
                  </div>
                  <div class="grid grid-cols-2">
                    <div class="px-4 py-2 font-semibold">Pembimbing Lapangan</div>
                    <div class="px-4 py-2">{{ item.tempatpkl.pembimbinglapangan?.nama }}</div>
                  </div>
                </div>
              </div>
              <div class="divider"></div>
